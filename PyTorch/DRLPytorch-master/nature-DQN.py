...
class Agent(object):
    ...
    def action(self, state, israndom):
        if israndom and random.random() < EPSILON:#贪婪侧率
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        if len(self.memory) > MEMORY_SIZE:
            self.memory.popleft() #删除最先进去的额
        if len(self.memory) < MEMORY_THRESHOLD:
            return  #小于的话不开始训练

        if self.learning_count % UPDATE_TIME == 0:    #间隔一定次数，两个target网络的的参数更新
            self.target_network.load_state_dict(self.network.state_dict())
        self.learning_count += 1

        batch = random.sample(self.memory, BATCH_SIZE)  #随机在self.memory选择size_batch
        #提取出
